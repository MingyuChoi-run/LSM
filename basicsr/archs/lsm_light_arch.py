...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
                 ...
                 rs_id=0, split_size=[2,4], max_phase=2*math.pi, shift_size=[1,2], reso=64, layerscale_value=1e-4,
                 d_state=16, rmin=0.9, rmax=0.99, num_tokens=128):
        super().__init__()
        ...
        if idx % 2 == 0:
            ...
            self.lru = SemanticLRU(
                dim, d_state, attn_drop=attn_drop, rmin=rmin, rmax=rmax, max_phase=max_phase, num_tokens=num_tokens, hidden_dim=mlp_hidden_dim
            )
            self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer)
