...
## LRU ##
class LRUScan(torch.autograd.Function):
    @staticmethod
    def forward(ctx, seq_1d, promptA, promptB, promptC, lambdas, B_re, B_im, C_re, C_im, D):
        with torch.no_grad():
...
def flops_lru_scan_fn(B=1, L=128*128, inF=180, outF=180, stateF=32):
    flops = 0
    # 1) Bu_re  = seq_1d @ B_re.T  → B×L×stateF inner products of length inF
    flops += 2 * B * L * inF * stateF
    # 2) Bu_im  = seq_1d @ B_im.T  (same cost)
    flops += 2 * B * L * inF * stateF
    # 3) real·prompt + imag·prompt → 2 multiplies per element
    flops += 2 * B * L * stateF
    # 4) prefix-scan (binary_operator_diag)
    flops += 6 * B * L * stateF
    # 5) real·promptC_re + imag·promptC_im
    flops += 2 * B * L * stateF
    # 6) C_re projection: inner_real (B,L,N) × C_re (outF,N)
    flops += 2 * B * L * outF * stateF
    # 7) C_im projection (same)
    flops += 2 * B * L * outF * stateF
    # 8) seq_1d @ D.T → B×L×outF inner products of length inF
    flops += 2 * B * L * inF * outF
    return flops

def lru_scan_flop_jit(inputs, outputs):
    x_shape = get_shape(inputs[0])
    promptA_shape = get_shape(inputs[1])
    out_shape = get_shape(outputs[0])
    B, L, inF = x_shape
    outF = out_shape[-1]
    stateF = promptA_shape[-1]
    return flops_lru_scan_fn(B, L, inF, outF, stateF)
...
class FLOPs:
    ...
    def register_supported_ops():
        ...
        supported_ops = {
            ...
            "prim::PythonOp.LRUScan": lru_scan_flop_jit,  # LRU
        }
